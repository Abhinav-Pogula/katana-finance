...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ visible, onClose, onSuccess, defaultType = 'expense' }) => {
  ...
  const checkAndNotifySavingsMilestone = async (
    ...
    for (const milestone of [...SAVINGS_MILESTONES].reverse()) {
      if (savingsRate >= milestone) {
        ...
        if (shouldNotify) await addSavingsMilestoneNotification(uid, savingsRate);
        break;
      }
    }
  };
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      const finalDate = parsedDate.isValid() ? parsedDate.format('YYYY-MM-DD') : dayjs().format('YYYY-MM-DD');

      const newTx: Omit<Transaction, 'id'> = {
        name: data.name?.trim() || selectedCategory.name, // use category name if empty
      ...
      if (uid) {
        ...
        if (newTx.type === 'expense') {
          ...
          if (budget) {
            ...
            const currentSpent = transactions
              .filter((tx) => tx.type === 'expense' && tx.category === newTx.category && dayjs(tx.date).isSame(dayjs(), 'month'))
            ...
            if (percentage >= 100) {
              ...
              if (shouldWarn) {
                Alert.alert('🚨 Budget Exceeded!', `You've exceeded your ${newTx.category} budget for this month!`);
              ...
              const shouldWarn = await checkAndSetWarningFlag(uid, flagKey);
              if (shouldWarn) {
                Alert.alert('⚠️ Budget Warning', `You've used ${Math.round(percentage)}% of your ${newTx.category} budget this month.`);
                await checkAndAddBudgetWarning(uid, newTx.category, percentage, month);
              }
            }
          }
        ...
        if (isLastDayOfMonth || isFirstDayOfMonth) {
          const targetMonth = isFirstDayOfMonth ? today.subtract(1, 'month').format('YYYY-MM') : today.format('YYYY-MM');
          const flagKey = `month_end_summary_${targetMonth}`;
          const shouldSend = await checkAndSetWarningFlag(uid, flagKey);
          if (shouldSend) {
            const monthTxs = transactions.filter((tx) => dayjs(tx.date).format('YYYY-MM') === targetMonth);
            const totalIncome = monthTxs.filter((tx) => tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0);
            const totalExpense = monthTxs.filter((tx) => tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0);
            ...
            await addMonthEndSummaryNotification(uid, targetMonth, totalIncome, totalExpense, topCategory);
          }
        }
      }
    } catch (error) {
          ...
          <View style={[
            ...
            >
              ...
              <View style={[styles.typeToggle, { backgroundColor: !isDark ? 'rgba(0,0,0,0.05)' : 'rgba(255,255,255,0.1)', borderRadius: s(16), padding: s(4), marginBottom: s(20) }]}>
                <TouchableOpacity 
                  style={[styles.toggleButton, { paddingVertical: s(12), borderRadius: s(12) }, activeType === 'expense' && { backgroundColor: colors.expense }]}
                  onPress={() => setValue('type', 'expense')}
                >
                  <Text style={[styles.toggleText, { color: activeType === 'expense' ? '#FFF' : colors.secondaryText, fontSize: s(14) }]}>Expense</Text>
                </TouchableOpacity>
                <TouchableOpacity 
                  style={[styles.toggleButton, { paddingVertical: s(12), borderRadius: s(12) }, activeType === 'income' && { backgroundColor: colors.income }]}
                  onPress={() => setValue('type', 'income')}
                >
                  <Text style={[styles.toggleText, { color: activeType === 'income' ? '#FFF' : colors.secondaryText, fontSize: s(14) }]}>Income</Text>
                </TouchableOpacity>
              </View>
              ...
              {/* Name Input — OPTIONAL */}
              <View style={[styles.inputGroup, { marginBottom: s(20) }]}>
                <Text style={[styles.label, { color: colors.secondaryText, fontSize: s(12), marginBottom: s(8) }]}>
                  What was it for? <Text style={{ color: colors.secondaryText, fontWeight: '400', textTransform: 'none', letterSpacing: 0 }}>(optional)</Text>
                </Text>
                <Controller
                  control={control}
                  name="name"
              ...
              {/* Date Input */}
              <View style={[styles.inputGroup, { marginBottom: s(20) }]}>
                <Text style={[styles.label, { color: colors.secondaryText, fontSize: s(12), marginBottom: s(8) }]}>Date</Text>
                <Controller
                  control={control}
                  name="date"
                  rules={{
                    validate: (value) => {
                      ...
                      return parsed.isValid() || 'Enter a valid date (YYYY-MM-DD)';
                    }
                  }}
                  render={({ field: { onChange, value } }) => (
                    <View style={[styles.dateInputContainer, {
                      backgroundColor: isDark ? '#1C1C1C' : '#F9F9F9',
                      borderColor: errors.date ? '#E74C3C' : (isDark ? '#333' : colors.divider),
                      borderWidth: 1,
                      borderRadius: s(12),
                      paddingHorizontal: s(12),
                      paddingVertical: s(12),
                    }]}>
                      <Ionicons name="calendar-outline" size={s(18)} color={colors.secondaryText} style={{ marginRight: s(8) }} />
                      <TextInput
                        style={[{ color: colors.text, fontSize: s(16), flex: 1 }]}
                ...
                />
                {errors.date && <Text style={[styles.errorText, { fontSize: s(12) }]}>{errors.date.message}</Text>}
              </View>
